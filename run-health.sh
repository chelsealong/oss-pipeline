...
if ! printf '%s' "$PROBLEMS" | grep -qE 'NONE dispatched|is not running|not loaded|no entries in the last hour|is PARTIAL|session-limit refusals|none merged in 7 days|sessions spent in 24h and no PR|cannot authenticate|runs failed in 24h'; then
