...
# History, because the number here has now moved twice for opposite reasons.
# The original floors (hermes>=15, the group>=10) stopped quota rationing from
# creeping back as a "cap". On 08-30 they were replaced by a resumption cap of 3
# after the credit stop. On 09-03 credit returned and Bruce restored the
# original numbers -- so the floors are live again, and what stops a burst is no
# longer a daily cap at all but the five-hour session ceiling in scan.py, which
# is what the subscription actually meters.
...
# Daily caps count what was OPENED. openclaw also limits what may be OPEN:
# openclaw-barnacle[bot] closes a PR when the author has more than 20 active,
...
# The burst guard itself: one ceiling, shared by both dispatchers, measured on
# the window the subscription actually uses.
spec = importlib.util.spec_from_file_location("sc", sys.argv[1] + "/scan.py")
sc = importlib.util.module_from_spec(spec); spec.loader.exec_module(sc)
if not hasattr(sc, "session_headroom"):
    print("  FAIL  no shared session ceiling — a burst can empty the 5h window"); bad += 1
else:
    ...
    for mod in ("watch.py", "watch-prs.py"):
        ...
        if "session_headroom" not in src:
            print(f"  FAIL  {mod} dispatches without consulting the session ceiling"); bad += 1
        if "note_session" not in src:
            print(f"  FAIL  {mod} does not record its sessions, so the other dispatcher cannot see them"); bad += 1
